...
class _ProductSearchState extends State<ProductSearch> {
  ...
  bool isSearch;
  bool isLoading;
  String currentText = 'Search Product By Keyword..';
                        ...
                        ? ListView.builder(
                            ...
                            itemBuilder: (context, pos) {
                              double different = double.parse(
                                      searchingList[pos]['mrpPrice']) -
                                  double.parse(searchingList[pos]['sellPrice']);
                              different = different *
                                  100 /
                                  double.parse(searchingList[pos]['mrpPrice']);
...
*/
  ...
  Future<Null> getData() async {
    ...
    final response = await http.get(
      UtilsData.kBaseUrl + UtilsData.kGetStationaryListMethod,
    ...
    if (response.statusCode == 200) {
      listInstitutes = data['products'];
      print('data of products :$listInstitutes');
      for (int i = 0; i < listInstitutes.length; i++) {
        var facultyData = {
          'id': listInstitutes[i]['id'],
          'name': listInstitutes[i]['name'],
          'thumbnailUrl': listInstitutes[i]['thumbnail_url'],
          'mrpPrice': listInstitutes[i]['mrp_price'],
          'sellPrice': listInstitutes[i]['sell_price'],
        };

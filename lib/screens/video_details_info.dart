...
import 'package:virtual_dukan/design_course/models/material_details.dart';
import 'package:virtual_dukan/material_info_screen.dart';
...
class VideoDetailsInfo extends StatefulWidget {
  ...
  VideoDetailsInfo({this.id, this.type});

  @override
  _VideoDetailsInfoState createState() => _VideoDetailsInfoState();
}
...
class _VideoDetailsInfoState extends State<VideoDetailsInfo>
  ...
  Future<MaterialDetailList> _future;
      ...
      child: Scaffold(
        ...
        body: FutureBuilder(
          ...
          builder: (BuildContext context,
              AsyncSnapshot<MaterialDetailList> snapshot) {
            if (!snapshot.hasData) {
              ...
              different =
                  different * 100 / double.parse(snapshot.data.course.mrpPrice);
              return Stack(
                children: <Widget>[
                  Column(
                    children: <Widget>[
                      AspectRatio(
                        aspectRatio: 1.2,
                        child: Image.network(snapshot.data.course.thumbnailUrl),
                      ),
                    ],
                  ),
                  Visibility(
                              ...
                              child: Scaffold(
                                  ...
                                  bottomNavigationBar: Visibility(
                                    visible: false /*(snapshot
                                                .data.course.typeOfCourse ==
                                            'free')
                                        ? false
                                        : (snapshot.data.course.typeOfCourse ==
                                                'purchased')
                                            ? false
                                            : true*/,
                                    child: InkWell(
                                                            ...
                                                            children: <Widget>[
                                                              ...
                                                              Text(
                                                                snapshot.data.course
                                                                    ...
                                                                    'free'
                                                                    ? 'Free'
                                                                    : '\₹${snapshot.data.course.salePrice}(${different.toStringAsFixed(2)}%)',
                                                                textAlign:
                                                                TextAlign
                                                                    .left,
                                                                style:
...
//                  ),
  ...
  void _lunchUrl({String url, int materialId, int instituteId}) async {
    if (await canLaunch(url)) {
      ...
      getMaterialVisitorData(instituteId: instituteId, materialId: materialId);
    } else {
      throw 'Could not lunch $url';
    }
  }

  Future<void> getMaterialVisitorData({int materialId, int instituteId}) async {
...
//    _progressDialog.show();
    ...
    final response = await http.post(
      ...
      body: jsonEncode(
        <String, String>{
          'type': 'material',
          'visited_id': materialId.toString(),
    ...
    var _responseData = jsonDecode(response.body);
    print('response of material visitor $_responseData');
...
//    }
  ...
  Future<MaterialDetailList> getData() async {
//    _progressDialog.setMessage(UtilsData.kLoadingMsg);
//    _progressDialog.show();
    SharedPreferences preferences = await SharedPreferences.getInstance();
    print('token ${preferences.getString(UtilsData.kToken)}');
    final response = await http.get(
      UtilsData.kBaseUrl + UtilsData.kMaterialDetailsMethod + '/${widget.id}',
      headers: <String, String>{
        UtilsData.kHeaderType: UtilsData.kHeaderValue,
        UtilsData.kAuthorization: preferences.getString(UtilsData.kToken),
      },
    );
    print('video details ${jsonDecode(response.body)}');
//      _progressDialog.hide();
    if (response.statusCode == 200) {
      return MaterialDetailList.fromJson(jsonDecode(response.body));
    } else if (response.statusCode == 401) {
      AwesomeDialog(
        context: context,
        dialogType: DialogType.WARNING,

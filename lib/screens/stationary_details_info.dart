...
class StationaryDetailsInfo extends StatefulWidget {
  final int id;
  final String profileImage;

  StationaryDetailsInfo({
    this.id,
    this.profileImage,
  });

  @override
  _StationaryDetailsInfoState createState() => _StationaryDetailsInfoState();
}

class _StationaryDetailsInfoState extends State<StationaryDetailsInfo>
      ...
      child: Scaffold(
        ...
        body: FutureBuilder(
          ...
          builder: (BuildContext context,
              AsyncSnapshot<StationaryDetailsListModel> snapshot) {
            if (!snapshot.hasData) {
...
//              double different = double.parse(snapshot.data.stationary.mrpPrice) -
//                  double.parse(snapshot.data.stationary.sellPrice);
//              different =
//                  different * 100 / double.parse(snapshot.data.stationary.mrpPrice);
              return Stack(
                children: <Widget>[
                  Column(
                    children: <Widget>[
...
//    }
  ...
  Future<StationaryDetailsListModel> getData() async {
...
//    _progressDialog.show();
    try {
      ...
      final response = await http.get(
        UtilsData.kBaseUrl +
            UtilsData.kStationaryDetailsMethod +
            '/${widget.id}',
...
//      _progressDialog.hide();
      if (response.statusCode == 200) {
        return StationaryDetailsListModel.fromJson(jsonDecode(response.body));
